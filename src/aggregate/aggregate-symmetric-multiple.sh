...
if [ $? -eq 0 ]; then
	~/Desktop/aggregate-symmetric individual_!(0.5)_*.csv
else
	echo "No data"
fi
echo ""
...
if [ $? -eq 0 ]; then
	~/Desktop/aggregate-symmetric majority_!(0.5)_*.csv
else
	echo "No data"
fi
echo ""
...
if [ $? -eq 0 ]; then
	~/Desktop/aggregate-symmetric quality_!(0.5)_*.csv
else
	echo "No data"
fi
echo ""
...
if [ $? -eq 0 ]; then
	~/Desktop/aggregate-symmetric random_!(0.5)_*.csv
else
	echo "No data"
fi